...
<table class='tbheader' border='0'><tr><td>
				...
				<td style='padding-left:15px;margin-top:10px;'><a href="giaodien/giohang.php"><img width='60px' src="hinhanh/icon/cart.png" /></a></td>  
				<td  style='padding-top: 10px'>
					<?php
					if (isset($_SESSION['nd_ma'])){		   
						$nd_ma = $_SESSION['nd_ma'];
							$sql="select * from nguoi_dung where nd_ma=$nd_ma";
							$result=mysql_query($sql);
							while($dong=mysql_fetch_array($result)){
								if($dong['nd_vip']==1){
									echo '<p style="margin-left:45px; color:#F88000">VIP: '.$_SESSION['nd_ten']."</p>";
								}else{
									echo '<p style="margin-left:35px;">Xin chào: '.$_SESSION['nd_ten']."</p>";
								}
							}
						echo '<a  style="margin-top:10px;" class="btdangnhap" href="giaodien/login/dx.php">Đăng Xuất</a>';
					}
					else{
