...
<?php	
	if($loai_ve=='all'){
		$sql="select * from chuyen_bay inner join tuyen_bay on tuyen_bay.tb_ma = chuyen_bay.tb_ma 
			...
			where sb_ma_di=$sb_ma_di and sb_ma_den=$sb_ma_den and cb_thoidiem='$ngay_di' and cb_tinhtrang='Còn'";
	}else{
		$sql="select * from chuyen_bay inner join tuyen_bay on tuyen_bay.tb_ma = chuyen_bay.tb_ma 
									   left join tuyenbay_khuyenmai on tuyen_bay.tb_ma =  tuyenbay_khuyenmai.tb_ma
									   left join khuyen_mai on khuyen_mai.km_ma = tuyenbay_khuyenmai.km_ma
                                       ...
                                       inner join ghe_chuyen on ghe_chuyen.lv_ma=loai_ve.lv_ma
															and ghe_chuyen.cb_ma=chuyen_bay.cb_ma
															and ghe_chuyen.mb_ma=chuyen_bay.mb_ma
			where sb_ma_di=$sb_ma_di and sb_ma_den=$sb_ma_den and cb_thoidiem='$ngay_di' and cb_tinhtrang='Còn' and loai_ve.lv_ma=$loai_ve";
	}
...
<table  border="1" class="tb1">
	...
	<td style="width:180px; border-right:1px solid rgb(225,225,225);padding-top:0px;">
		<?php 	if($km_giatri<>1){	echo '<p class="km">'.((1-$km_giatri)*100).'%</p>'; echo '<p class="giagoc">'.number_format($dong['gia']*$_SESSION['tg_tigia']).' '.$_SESSION['tg_donvi'].'</p>'; }		?>
...
<?php	// 
	if($loai_ve=='all'){
		$sql="select * from chuyen_bay inner join tuyen_bay on tuyen_bay.tb_ma = chuyen_bay.tb_ma
			...
			where sb_ma_di=$sb_ma_den and sb_ma_den=$sb_ma_di and cb_thoidiem='$ngay_ve' and cb_tinhtrang='Còn'";
	}else{
		$sql="select * from chuyen_bay inner join tuyen_bay on tuyen_bay.tb_ma = chuyen_bay.tb_ma 
									   left join tuyenbay_khuyenmai on tuyen_bay.tb_ma =  tuyenbay_khuyenmai.tb_ma
									   left join khuyen_mai on khuyen_mai.km_ma = tuyenbay_khuyenmai.km_ma
									   ...
									   and ghe_chuyen.cb_ma=chuyen_bay.cb_ma
                                       and ghe_chuyen.mb_ma=may_bay.mb_ma
			where sb_ma_di=$sb_ma_den and sb_ma_den=$sb_ma_di and cb_thoidiem='$ngay_ve' and cb_tinhtrang='Còn' and loai_ve.lv_ma=$loai_ve";
	}
	$row=mysql_query($sql);
